...
<% else_if $MediaType == 'video' && $MediaVideoFullURL && $MediaVideoEmbeddedURL %>
    <% if $MediaCaption %><div class="captionImage"><% end_if %>
        <figure class="video image<% if $BulmaRatio %> $BulmaRatio<% end_if %>" data-video-embed-url="$MediaVideoEmbeddedURL" data-element-id="$ID" data-video-type="$MediaVideoProvider"<% if $LoopVideo %> data-loop-video="1"<% end_if%><% if $AutoplayVideo %> data-autoplay-video="1"<% end_if %>>
